...
def list_img(image_dir: str, ext='.png'):
    """
    ...
    lst_images = [file_name for file_name in files if file_name.endswith(ext)]
    return lst_images
...
def load_data_masks(image_dirs, labels, new_size=(28, 28), limits=None, random_state=None):
    ...
    Args:
        image_dirs (list of lists of 'str'): the paths to folders.
            ...
            Rhe function attempts to match all folders
    ...
    for idx, modality in enumerate(image_dirs):
        ...
        all_file_mames.append(fnames)

    data = np.concatenate(all_images, axis=3)
    return data, label_data, all_file_mames
