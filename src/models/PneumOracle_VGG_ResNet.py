...
min_train_samples = min(vgg_train_features.shape[0], resnet_train_features.shape[0])
min_val_samples = min(vgg_val_features.shape[0], resnet_val_features.shape[0])
...
print("Combinaison des caractéristiques de VGG16 et ResNet50...")
X_train = np.concatenate([vgg_train_features, resnet_train_features], axis=1)
X_val = np.concatenate([vgg_val_features, resnet_val_features], axis=1)
...
def predict_and_display_images(test_image_dir):
    ...
    for file_name in os.listdir(test_image_dir):
        if file_name.endswith('.png'):
            ...
            combined_features = np.concatenate([vgg_features, resnet_features], axis=1)
...
Accuracy: 0.7657700547488693
Classification Report:
                 precision    recall  f1-score   support

          COVID       0.80      0.32      0.46       697
   Lung Opacity       0.71      0.78      0.74      1177
         Normal       0.79      0.90      0.84      2065
Viral Pneumonia       0.82      0.78      0.80       262

       accuracy                           0.77      4201
      macro avg       0.78      0.70      0.71      4201
   weighted avg       0.77      0.77      0.75      4201

AUC-ROC: 0.9263921067120194
F1 Score: 0.7481897004600353'''