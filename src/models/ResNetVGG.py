...
from tensorflow.keras.layers import Input, Dense, Dropout, GlobalAveragePooling2D, Concatenate, Resizing
...
# Paramètres
IMG_HEIGHT = 256  # Taille augmentée de l'image pour plus de détails
IMG_WIDTH = 256   # Taille augmentée de l'image pour plus de détails
NUM_CLASSES = 4
...
# Définir le modèle ResNet50 combiné avec VGG16
def resnet_vgg_combined_model(input_size=(IMG_HEIGHT, IMG_WIDTH, 3), num_classes=NUM_CLASSES):
    inputs = Input(input_size)

    # Charger les modèles pré-entraînés
    resnet_base = ResNet50(include_top=False, weights='imagenet', input_tensor=inputs)
    vgg_base = VGG16(include_top=False, weights='imagenet', input_tensor=inputs)

    # Extraire les sorties intermédiaires
    resnet_output = resnet_base.output
    vgg_output = vgg_base.output

    # Redimensionner les sorties de VGG pour correspondre à celles de ResNet
    vgg_output_resized = Resizing(resnet_output.shape[1], resnet_output.shape[2])(vgg_output)

    # Fusionner les sorties
    combined = Concatenate()([resnet_output, vgg_output_resized])
...
# Créer et compiler le modèle combiné
print("Création et compilation du modèle combiné ResNet50 et VGG16...")
model = resnet_vgg_combined_model()
...
def predict_and_display_images(test_image_dir, test_mask_dir):
    model = tf.keras.models.load_model('resnet_vgg_combined_model.keras')
